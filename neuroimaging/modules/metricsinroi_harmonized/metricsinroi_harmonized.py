"""MultiQC module for metricsinroi_harmonized results.
...
class MultiqcModule(BaseMultiqcModule):
    ...
    def __init__(self):
        super(MultiqcModule, self).__init__(
            name="Metrics in region of interest harmonized to a reference site",
            anchor="metricsinroi_harmonized",
        ...
        files = list(self.find_log_files("metricsinroi_harmonized"))

        # Nothing found - raise ModuleNoSamplesFound to tell MultiQC
        if len(files) == 0:
            log.debug(f"Could not find metricsinroi_harmonized reports in {config.analysis_dir}")
            raise ModuleNoSamplesFound
        ...
        config_thresh = getattr(config, "metricsinroi_harmonized", {})
        ...
        self.write_data_file(
            {"rois": roi_metrics},
            "multiqc_metricsinroi_harmonized",
        )
...
Pass: within Q1 - {iqr_multiplier}*IQR to Q3 + {iqr_multiplier}*IQR range for all ROIs, Fail: outside range in at least one ROI"""
            ...
            self.add_section(
                ...
                anchor=f"metricsinroi_harmonized-{metric_key}",
                description=description_html,
                plot=violin.plot(
                    plot_data,
                    headers=headers,
                    pconfig={
                        "id": f"metricsinroi_harmonized_{metric_key}_violin",
                        "title": f"MetricsInROI harmonized: {metric_cfg['title']}",
                    },
                ),
