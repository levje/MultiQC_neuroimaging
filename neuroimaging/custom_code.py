...
def neuroimaging_execution_start():
    ...
    if "metricsinroi" not in config.sp:
        config.update_dict(config.sp, {"metricsinroi": {"fn": "space-native*mean_desc-roi_stats.tsv"}})

    if "metricsinroi_harmonized" not in config.sp:
        config.update_dict(config.sp, {"metricsinroi_harmonized": {"fn": "space-harmonized*mean_desc-roi_stats.tsv"}})
